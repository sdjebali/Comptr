...
(* Print an error but does not quit *)
let print_pb s =
...
let make_temp_file_name file =
  let u = Random.self_init and time = Unix.localtime (Unix.time ()) and pid = Unix.getpid () in 
    (("/tmp/")^(string_of_int (1+time.Unix.tm_mon))^("_")^(string_of_int time.Unix.tm_mday)^("_")^(string_of_int time.Unix.tm_hour)^"_"^(string_of_int time.Unix.tm_min)^("_")^(string_of_int time.Unix.tm_sec)^("_")^(string_of_int pid)^("_")^(string_of_int (Random.int 1000000)));;
...
(* in order to always have something in the string (here a dot) even when list is empty *)
